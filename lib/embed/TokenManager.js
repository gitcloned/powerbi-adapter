...
class TokenBuilder {
    ...
    generate(accessToken, params) {
        ...
        if ((this._payload.reports && this._payload.reports.length == 1
            && this._payload.targetWorkspaces && this._payload.targetWorkspaces.length == 1)) {
        ...
        return new Promise((resolve, reject) => {
            ...
            request(options, (error, response) => {
                ...
                reject(this.errorGeneratingToken(accessToken, params, response))
            })
        }).then(result => {
    ...
    errorGeneratingToken(accessToken, params, response) {
            ...
            if (response.body) {
                ...
                try {
                    ...
                    if (error.code == "TokenExpired") {
                        ...
                        return { error, accessTokenExpired: true, statusCode: response.statusCode }
                    }
                } catch (e) { }
            }
        }

        return { error: response.body || `${response.statusCode}: some internal error occurred while generating embed token`, accessTokenExpired: false, statusCode: response.statusCode }
    }
}